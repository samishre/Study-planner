...
def generate_schedule_view(request):
    if request.method == 'POST':
        ...
        UserAvailability.objects.create(user=user, daily_hours=available_hours_per_day)

        # Create topics and topic map
        topic_map = {}
        ...
        end_of_study = exam_dt - timedelta(days=1)
        ...
        for item in gemini_response:
            ...
            if not topic_obj:
                print(f"⚠ Topic not matched in topic_map: {topic_title}")
                continue
            ...
            StudySchedule.objects.create(
                ...
                study_date=today,
                start_time=now,
                end_time=datetime.combine(end_of_study, datetime.max.time()),
